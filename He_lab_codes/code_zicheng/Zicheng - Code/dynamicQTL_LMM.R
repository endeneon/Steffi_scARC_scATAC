...
####################Final dynamic QTL input preparation
library(tidyverse)
library(vroom)
library(Matrix)
library(RNOmni)
...
logTPM_counts = function(cluster){
  ...
  count_matrix = Matrix(as.matrix(count_matrix),sparse = T)
...
################dynamic QTL test with permutations
library(MASS)
library(RhpcBLASctl)
library(tidyverse)
library(vroom)
...
#unique_peak = lapply(topQTL_list, function(x) x$phenotype_id) %>% Reduce(union,.)
...
permutation_LM = function(iter,QTL_list,atac_mtx,snp_mtx,time_col,covar_mtx){
  ...
  mod = lm(atac ~ snp + time + snp:time + Geno_PC1*time + Geno_PC2*time + Geno_PC3*time +
             PC1*time + PC2*time + PC3*time + PC4*time + PC5*time,
           data = reg_df)
  ...
  names(emp_pvals) = c("emp_1hr_pval",
                       "emp_6hr_pval",
                       "emp_anova_pval",)
  return(list(coef = reg_coef, anova_res = anova_res,
              emp_pvals = emp_pvals))
}
...
dynamic_qtl_fdr = lapply(dynamic_qtl_perm, function(x){
  ...
  colnames(fdr_df) = c("emp_1hr_qval",
                    "emp_6hr_qval",
                    "emp_anova_qval",)
  cbind(x[,1:2],fdr_df)
})
