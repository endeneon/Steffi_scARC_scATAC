...
arrow_proj@sampleColData = arrow_proj@sampleColData[grepl("Batch_024",rownames(arrow_proj@sampleColData), fixed = T),,drop = F]
arrow_proj@sampleMetadata@listData = arrow_proj@sampleMetadata@listData[grepl("Batch_024",names(arrow_proj@sampleMetadata@listData), fixed = T)]
...
lapply(unique_batch, function(batch){
  cell_frags = getFragmentsFromArrow(ArrowFile = paste0("/scratch/midway3/zichengwang/ArchR_subset/ArrowFiles/",batch,".arrow"),
                                     verbose = F)
  ...
  lapply(c("GABA","nmglut","npglut"), function(cell_type){
    context_frags = cell_frags[cell_frags$RG %in% subset_cellnames[subset_cell_meta$cell_type == cell_type]] %>% as.data.table() %>%
      .[,1:3] %>% .[,start:= start-1L] 
    ...
    write_tsv(context_tagAlign, file = con, col_names = F)
    close(con)
...
config$ATAC = sapply(paste0(rep(c(0,1,6),3),"_",c(rep("GABA",3),rep("nmglut",3),rep("npglut",3))), function(x){
  list.files("/scratch/midway3/zichengwang/tagAlign_files", pattern = paste0(".*",x,"_tagAlign.gz$"),full.names = T) %>% paste0(.,collapse = ",")
})
...
plotEmbedding(ArchR_subset, name = "cell_cluster", embedding = "UMAP_Harmony_Combined",
              size = 1.5, labelAsFactors=F, labelMeans=F, width = 5, height = 5)

saveArchRProject(ArchR_subset)

##################pseudo time trajactory
library(ArchR)
addArchRThreads(8)
ArchR_subset = loadArchRProject("/scratch/midway3/zichengwang/ArchR_subset/")
