...
######### create combined mtx by chr for gene peak mapping
library(tidyverse)
library(Seurat)
library(Matrix)

#selected_genes = readRDS("/project/xinhe/zicheng/neuron_stimulation/data/scGLM_genes.rds")
gene2peakPairs = readRDS("/project/xinhe/zicheng/neuron_stimulation/data/Gene2Peak_500kb.rds")
cellMeta = readRDS("/project/xinhe/zicheng/neuron_stimulation/data/cellMeta.rds") %>% 
  as.data.frame() %>% filter(grepl("Batch_024",batch_wo_time))
...
create_scGLM_mtx = function(cell_type){
  ...
  save(subset_cellMeta,Peak2GeneList,rna_agg,atac_agg,mm_agg,
       file = con)
  close(con)
...
create_reg_input = function(cell_type, chr , time = "all", max_row = 5000, threshold = 0.05){
  ...
  atac = atac[rowMeans(atac > 0) >= 0.05,]
  ...
  #save(subset_cellMeta,atac,rna,file = paste0("/scratch/midway3/zichengwang/Temp_GenePeakMtx/",cell_type,"_","_chr",chr,"_",time,".RData"),compress = TRUE)
  ...
  mclapply(names(split_matrices), function(split) {
    ...
    assoc_obj = list(Peak2GeneList = split_matrices[[split]],
                                            cellMeta = subset_cellMeta,
                                            atac_mtx = subset_atac,
                                            mrna_mtx = subset_rna)
    save(list = names(assoc_obj),
         file = paste0("/scratch/midway3/zichengwang/Temp_GenePeakMtx/",time,"_",cell_type,"_",split,".RData"),
         envir = list2env(assoc_obj),compress = TRUE)
  },mc.cores = 12)
...
batch_names = names(unlist(reg_batch_list,recursive = F)) %>% gsub(".","_",.,fixed = T)
