...
names(fragments_files) <-
  paste0(
    ...
    str_replace(
      ...
      pattern = "/outs/atac_fragments.tsv.gz",
      replacement = ""
    )
  )

# find all barcodes.tsv.gz files in the current working directory and its subdirectories
csv_barcode_files <-
  list.files(
    path = multiome_bams_dir,
...
names(csv_barcode_files) <-
  paste0(
    ...
    str_replace(
      ...
      pattern = "/outs/filtered_feature_bc_matrix/barcodes.tsv.gz",
      replacement = ""
    )
  )
...
names(h5_files) <-
  paste0(
    ...
    str_replace(
      ...
      pattern = "/outs/filtered_feature_bc_matrix.h5",
      replacement = ""
    )
  )
...
if (
  ...
  print(
    "All fragments.tsv.gz files have corresponding barcodes.tsv.gz files and h5 files."
  )
} else {
  stop("Mismatch between fragments.tsv.gz, barcodes.tsv.gz, and h5 files.")
}
...
filtered_barcode_list <-
  ArchR::getValidBarcodes(
    csvFiles = c(
      "/home/szhang37/CAB_workspace/projects/szhang_dev/test_ASoC_w_WASP/atac_bams/24t/outs/singlecell.csv",
      "/home/szhang37/CAB_workspace/projects/szhang_dev/test_ASoC_w_WASP/atac_bams/pt12pre/outs/singlecell.csv"
    ),
    sampleNames = c("24t", "pt12pre")
  )
filtered_barcode_list@listData <- list() # clean up the listData slot
...
projMultiome <-
  ArchR::ArchRProject(
    ...
    outputDirectory = "ArchR_multiome_obj",
    copyArrows = TRUE
  )
projMultiome <-
  saveArchRProject(
...
# load in the gene expression matrices from the h5 files and add them to the ArchRProject
## load the gene expression matrices from the h5 files
...
# pre-cleanup the gene expression matrices to remove any cells that are not in the ArchRProject and vice versa
print(paste(
  length(which(
    getCellNames(projMultiome) %ni% colnames(gene_expression_matrices)
  )),
  "cells in the ArchRProject do not have matching gene expression data."
)) # note the %ni% operator is defined in ArchR package, 0 means all matched
cellsToKeep <- which(
  getCellNames(projMultiome) %in% colnames(gene_expression_matrices)
)
projMultiome <-
  subsetArchRProject(
    ArchRProj = projMultiome,
    cells = getCellNames(projMultiome)[cellsToKeep],
    outputDirectory = "ArchR_multiome_obj",
    force = TRUE
  )

# add the gene expression matrices to the ArchRProject
projMultiome <-
  addGeneExpressionMatrix(
    input = projMultiome,
    seRNA = gene_expression_matrices,
    excludeChr = c("chrM", "chrX", "chrY"),
    # ArchR's .validInput only accepts NULL or a BatchtoolsParam for
    # parallelParam; NULL lets ArchR parallelize via addArchRThreads().
    parallelParam = NULL,
    strictMatch = TRUE,
    force = TRUE
  )
cat(paste0(
  "Gene expression matrices added to ArchRProject for ",
  length(arrow_files),
  " samples with cell numbers: \n",
  {
    cells_per_sample <-
...
apply_filtering_switch <- TRUE # set to TRUE to filter doublets, FALSE to skip filtering
...
# Iterative LSI on the scRNA-seq (GeneExpressionMatrix). RNA LSI needs different
# settings than ATAC: select variable genes (firstSelection = "variable"), do NOT
# binarize counts, and use the RNA depth column "Gex_nUMI" instead of "nFrags".
print(paste0(
  "Adding Iterative LSI (RNA) for ",
  length(arrow_files),
  " samples..."
))
projMultiome <-
...
print("Iterative LSI (RNA) added and ArchRProject saved successfully.")

# Combine the ATAC ("IterativeLSI") and RNA ("LSI_RNA") reduced dimensions into a
# single multiomic reduced-dims object for joint clustering/embedding.
print("Adding combined ATAC + RNA reduced dimensions (LSI_Combined)...")
projMultiome <-
  ArchR::addCombinedDims(
    ArchRProj = projMultiome,
...
print(
  "Combined reduced dimensions (LSI_Combined) added and ArchRProject saved successfully."
)

print(paste0("Adding Harmony for ", length(arrow_files), " samples..."))
print(
  "!! Switching to LSI_RNA reducedDims for Harmony clustering, tSNE, UMAP, and peak calling !!"
)
projMultiome <-
  ArchR::addHarmony(
    ArchRProj = projMultiome,
