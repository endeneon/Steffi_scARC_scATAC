...
# head(rownames(projAtac@cellColData))
...
# sum(rownames(projAtac@cellColData) %in% barcodes_w_ident)
# # add transferred_barcode column to cellColData and fill it with NA
# projAtac@cellColData$transferred_barcode <- NA
# projAtac@cellColData$transferred_barcode[
#   rownames(projAtac@cellColData) %in% barcodes_w_ident
# ] <- "Hepatocytes"
...
# merge projAtac + projPeakSource, add PeakMatrix, save to new dir ####
# ArchR has no direct project-merge function, so we build a fresh ArchRProject
# from the union of both projects' Arrow files. Arrow files carry their own
# per-cell data, so the new project inherits all cells from both inputs.
{
  merged_output_dir <- "ArchR_merged_ATAC_multiome_obj"

  # collect and de-duplicate Arrow files from both source projects
  merged_arrow_files <-
    unique(c(
      getArrowFiles(projAtac),
      getArrowFiles(projPeakSource)
    ))
  print(paste0(
    "Merging ",
    length(merged_arrow_files),
    " unique Arrow files from projAtac and projPeakSource."
  ))

  # build the merged project in the target output directory
  projMerged <-
    ArchRProject(
      ArrowFiles = merged_arrow_files,
      outputDirectory = merged_output_dir,
      copyArrows = TRUE,
      showLogo = FALSE
    )
  print(paste0(
    "Merged ArchRProject created with ",
    nCells(projMerged),
    " cells."
  ))

  # transfer the peakSet from projPeakSource onto the merged project
  projMerged <-
    addPeakSet(
      ArchRProj = projMerged,
      peakSet = getPeakSet(projPeakSource),
      force = TRUE
    )
  print(paste0(
    "peakSet from projPeakSource added (",
    length(getPeakSet(projMerged)),
    " peaks)."
  ))

  # compute the PeakMatrix over the merged cells using that peakSet
  projMerged <-
    addPeakMatrix(
      ArchRProj = projMerged,
      force = TRUE
    )
  print("PeakMatrix added to merged project.")
  print(getAvailableMatrices(projMerged))

  # persist the merged project
  projMerged <-
    saveArchRProject(
      ArchRProj = projMerged,
      outputDirectory = merged_output_dir,
