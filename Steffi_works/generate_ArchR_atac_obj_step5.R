...
# Some Arrow files have cells in Metadata/CellNames (hence in cellColData) that
# are absent from the TileMatrix; those phantom cells crash addIterativeLSI's
# .getPartialMatrix ("not all cellNames found in partialMatrix"). Keep only cells
# that actually have TileMatrix data.
tile_cells <-
  unlist(
    lapply(getArrowFiles(projMerged), function(af) {
      samp <- as.character(rhdf5::h5read(af, "Metadata/Sample"))[1]
      paste0(samp, "#", as.character(rhdf5::h5read(af, "TileMatrix/Info/CellNames")))
    }),
    use.names = FALSE
  )
rhdf5::h5closeAll()

keep_cells <- intersect(getCellNames(projMerged), tile_cells)
print(paste0(
  "Dropping ",
  length(getCellNames(projMerged)) - length(keep_cells),
  " cells missing from TileMatrix before LSI."
))
if (length(keep_cells) < length(getCellNames(projMerged))) {
  projMerged <-
    subsetArchRProject(
      ArchRProj = projMerged,
      cells = keep_cells,
      outputDirectory = paste0(merged_output_dir, "_subset"),
      dropCells = TRUE,
      force = TRUE
    )
}
...
projMerged <-
  ArchR::addHarmony(
    ArchRProj = projMerged,
    reducedDims = "IterativeLSI_Tiles",
    name = "Harmony_Tiles",
    groupBy = "Sample",
    force = TRUE
  )
...
add_clusters_at_resolution <-
  function(
    ...
    cluster_name <- str_c("Cluster_", resolution_value, "_Tiles")
    print(paste0(
      "Adding Clusters '",
      cluster_name,
      "' at resolution ",
