#! /usr/bin/env Rscript
# Use TileMatrix to subset cells
# init
{
  library(Seurat)
  library(gplots)
...
## 2D density map of Macrophage in UMAP_Tiles space ####
...
# align cellColData and subset to Macrophage
...
density_plot <-
  ...
  labs(
    title = "UMAP_Tiles 2D density: Macrophage",
    x = "UMAP 1",
    y = "UMAP 2",
    fill = "Density"
  ) +
...
kde_bandwidth <- c(
  MASS::bandwidth.nrd(macro_df$UMAP1),
  MASS::bandwidth.nrd(macro_df$UMAP2)
)
...
plotEmbedding(
  ...
  colorBy = "cellColData",
  name = "tiles_projected_macrophages",
  size = 0.1,
  # sampleCells = 10000,
  highlightCells = NULL,
  rastr = FALSE,
