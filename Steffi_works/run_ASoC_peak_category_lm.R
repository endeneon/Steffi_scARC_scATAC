...
# Per-SNP ASoC peak x sample-category association in hepatocytes.
#
# This is a category (Primary vs Resistant) variant of
# run_ASoC_peak_genotype_lm.R. For every SNP in
# hepatocyte_ASoC_genotypes_GT_summary_min3.tsv:
...
#   * regress the per-sample RPGC on the sample's category
#     (projHepatocytes@cellColData$category, factored with "Resistant" as the
#     reference level) and also run a two-group Wilcoxon rank-sum test
#   * write the R^2 / p-value table and one box-and-whisker panel per peak,
#     Dark2 fill, with the Wilcoxon significance labelled at the top of the panel
#     and the linear-regression R^2/p in the panel title
#
# The SNP -> peak pairing (and therefore the per-panel layout) is kept identical
# to the genotype script so panels line up 1:1; genotype is retained only as a
# panel-title annotation, not as a model term.
#
# init
{
  library(ArchR)
...
# "Primary" is the reference level for the category factor.
...
# Genotype is only used for the panel-title annotation here, not for any model.
normalise_gt <-
  function(gt) {
    vapply(
...
# Significance stars for a p-value; used for the on-panel Wilcoxon label.
.p_stars <-
  function(p) {
...
fit_one <-
  function(i) {
    ...
    d <- data.frame(
      sample = use_samples,
      category = factor(
        unname(sample_category[use_samples]),
        levels = category_levels
      ),
      genotype = factor(gt, levels = gt_levels),
      rpgc = as.numeric(mat_rpgc[peak_i, use_samples]),
      stringsAsFactors = FALSE
    )
    d <- d[!is.na(d$category) & is.finite(d$rpgc), , drop = FALSE]
    ...
    res <- data.frame(
      ...
      beta_primary = NA_real_,
      r_squared_lm = NA_real_,
      p_value_lm = NA_real_,
    ...
    d_fac <- d
    d_fac$category <- droplevels(d_fac$category)
    fit_lm <- stats::lm(rpgc ~ category, data = d_fac)
    coef_name <- grep("^category", names(stats::coef(fit_lm)), value = TRUE)[1]
    if (!is.na(coef_name)) {
      res$beta_primary <- unname(stats::coef(fit_lm)[[coef_name]])
    }
    res$r_squared_lm <- summary(fit_lm)$r.squared
    res$p_value_lm <- .model_p(fit_lm)

    res$p_value_wilcox <- tryCatch(
      stats::wilcox.test(rpgc ~ category, data = d_fac, exact = FALSE)$p.value,
...
df_results$fdr_lm <- stats::p.adjust(df_results$p_value_lm, method = "BH")
df_results$fdr_wilcox <- stats::p.adjust(
  df_results$p_value_wilcox,
  method = "BH"
)
df_results <- df_results[
  order(df_results$p_value_lm, na.last = TRUE),
  ,
  drop = FALSE
]
...
write.table(
  df_results,
  file.path(writeout_dir, "ASoC_peak_category_lm_results.tsv"),
  sep = "\t",
  quote = FALSE,
  row.names = FALSE
)
qs2::qs_save(
  df_results,
  file.path(writeout_dir, "ASoC_peak_category_lm_results.qs2"),
...
panel_titles <- vapply(
  ...
  function(st) {
    sprintf(
      ...
      st$peak_id,
      ifelse(
        is.na(st$r_squared_lm),
        "NA",
        formatC(st$r_squared_lm, format = "f", digits = 3)
      ),
      ifelse(
        is.na(st$p_value_lm),
        "NA",
...
page_files <-
  ...
  ) %dopar%
  {
    ...
    panels <- lapply(idx, function(k) {
      d <- plot_data[[k]]
      d$category <- droplevels(d$category)
      mu <- stats::aggregate(rpgc ~ category, data = d, FUN = mean)

      st <- plot_stats[[k]]
      p_wil <- st$p_value_wilcox

      # Wilcoxon significance bracket above the boxes.
      y_max <- max(d$rpgc, na.rm = TRUE)
      y_min <- min(d$rpgc, na.rm = TRUE)
      y_rng <- y_max - y_min
      if (!is.finite(y_rng) || y_rng == 0) {
        y_rng <- abs(y_max) %||% 1
      }
      y_bracket <- y_max + 0.08 * y_rng
      y_label <- y_max + 0.16 * y_rng
      ...
      p <- ggplot2::ggplot(d, ggplot2::aes(x = category, y = rpgc)) +
      ...
      if (show_bracket) {
        p <- p +
          ...
          ggplot2::expand_limits(y = y_label + 0.06 * y_rng)
      }
