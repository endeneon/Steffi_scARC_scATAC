...
## transfer barcodes from annotated GEX object
selected_cells_obj <-
  qs_read(
...
barcodes_w_ident <-
  barcodes_w_ident[
    as.character(selected_cells_obj$celltype_broad) %in% c("Macrophage")
  ]
barcodes_w_ident <-
  sub(
    "^([^_]*__[^_]*)_",
...
# align cellColData and subset to Macrophages
...
unique(umap_df$transferred_barcode)
mac_df <- umap_df[
  !is.na(umap_df$transferred_barcode) &
    umap_df$transferred_barcode == "Macrophage",
]

lower_density_cutoff <- 0.04
density_plot <-
  ggplot(
    mac_df,
    aes(x = UMAP1, y = UMAP2)
  ...
  labs(
    title = "UMAP_Tiles 2D density: Macrophages",
    x = "UMAP 1",
    y = "UMAP 2",
    fill = "Density"
  ) +
...
projMerged@cellColData$tiles_projected_macrophages <- FALSE

# --- Project the Macrophage density coverage onto all cells ---------------
...
kde_bandwidth <- c(
  MASS::bandwidth.nrd(mac_df$UMAP1),
  MASS::bandwidth.nrd(mac_df$UMAP2)
)

# fit the density on Macrophage cells over a grid spanning ALL cells so that
# non-Macrophage barcodes inside the region can also be evaluated
kde_fit <- MASS::kde2d(
  x = mac_df$UMAP1,
  y = mac_df$UMAP2,
  h = kde_bandwidth,
...
projMerged@cellColData$tiles_projected_macrophages <- coverage_flag

# sum(
#   rownames(projMerged@cellColData)[
#     rownames(projMerged@cellColData)[
#       (projMerged@cellColData$tiles_projected_macrophages)
#     ] %in%
#       rownames(projMerged@cellColData)[
#         (projMerged@cellColData$peaks_projected_macrophages)
#       ]
#   ] %in%
#     rownames(projMerged@cellColData)[
...
##   !(projMerged@cellColData$transferred_barcode %in% "Macrophage")
## ])
# length(rownames(projMerged@cellColData))
## names((projMerged@cellColData$tiles_projected_macrophages))

# repeat with Peaks instead of tiles
plotEmbedding(
  ArchRProj = projMerged,
  embedding = "UMAP_Peaks",
  colorBy = "cellColData",
...
density_plot <-
  ...
  geom_point(size = 0.1, alpha = 0.2, color = "grey20") +
  labs(
    title = "UMAP_Peaks 2D density: Macrophages",
    x = "UMAP 1",
    y = "UMAP 2",
    fill = "Density"
  ) +
...
projMerged@cellColData$peaks_projected_macrophages <- FALSE
...
coverage_flag <- rownames(projMerged@cellColData) %in% inside_barcodes
projMerged@cellColData$peaks_projected_macrophages <- coverage_flag

table(projMerged@cellColData$tiles_projected_macrophages)
table(projMerged@cellColData$peaks_projected_macrophages)
...
#       ]
#   ]
length(confident_macrophage_barcodes)
qs_save(
  confident_macrophage_barcodes,
