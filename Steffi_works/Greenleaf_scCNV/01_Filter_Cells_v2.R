...
# Prefer forking (multicore) on Linux/LSF so the large `fragments` GRanges is
# shared copy-on-write across workers instead of being serialized to each one.
...
feature <- txdb %>%
  ...
  unique
# Smaller batches -> more independent tasks to keep all 16 workers busy and to
