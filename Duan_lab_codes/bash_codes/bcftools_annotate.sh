...
dbsnp="/home/zhangs3/Data/Databases/Genomes/hg38/dbsnp_154.hg38.vcf.gz"
...
## annotate variants with rsID

# $gatk4_path/gatk VariantAnnotator \
#	-V output/$output_cell_type$output_cell_time"_merged_SNP_VF.vcf" \
#	-R $hg38_index \
#	--dbsnp $dbsnp \
#        -O output/$output_cell_type$output_cell_time"_annotated.vcf"	
...
bcftools annotate \
	...
	-O v \
	--threads 16 \
	output/$output_cell_type$output_cell_time"_merged_SNP_VF.vcf.gz"


cat <(cat output/$output_cell_type$output_cell_time"_annotated.vcf" \
	| grep "^#") \
    <(cat output/$output_cell_type$output_cell_time"_annotated.vcf" \
        | grep -v "^#" \
	| grep "rs[0-9].*") \
    > output/$output_cell_type$output_cell_time"_annotated_rs_only.vcf"
...
#        | sed 's/variant.\.AD/variant\tAD/g' \
#        | sed 's/variant..\.AD/variant\tAD/g' \
#	> output/$output_cell_type$output_cell_time"_scATAC_direct_count_28Dec2022_4_R.txt"
#
