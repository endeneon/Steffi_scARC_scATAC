...
load("/nvmefs/scARC_Duan_018/Duan_project_024_ATAC/no_rat_no_unmapped_no_rat_gene_clean_list.RData")
...
df_raw_filtered_table <-
  data.frame(library = sample_index,
             ...
             filtered.cell.counts = rep_len(0, length(sample_index)),
             filtered.fragment.count = rep_len(0, length(sample_index)),
             filtered.fragment.per.cell = rep_len(0, length(sample_index)),
...
for (i in 1:length(sample_index)){
  ...
  print(unique(cleanobj_lst[[i]]$cell.line.ident))
...
write.table(df_raw_filtered_table, 
            file = "./cell_and_reads_count_QC/ATAC_read_counts_from_fragment_file_raw_filtered.csv", 
            quote = F, sep = ",", row.names = F, col.names = T)

# plot the cell count ####
count_table <- read_csv("/nvmefs/scARC_Duan_018/Duan_project_024_ATAC/cell_and_reads_count_QC/ATAC_read_counts_from_fragment_file_raw_filtered.csv")
