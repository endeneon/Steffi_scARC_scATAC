...
input_file_list <-
  list.files(path = "no_upsampling_20Sept2022", 
             pattern = "20Sept2022\\.txt$",
             full.names = T)
output_path <- "no_upsampling_20Sept2022/output/"
...
for (i in 1:length(input_file_list)) {
  ...
  output_file_full_name <-
    str_remove_all(string = output_file_full_name,
                   pattern = '_downsampled_median_downsampled_no_upsampling_20Sept2022')
