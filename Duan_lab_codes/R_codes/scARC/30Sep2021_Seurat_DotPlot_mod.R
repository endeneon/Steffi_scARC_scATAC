# Chcuxuan Li 09/30/2021
library(RColorBrewer)
library(viridis)
library(ggplot2)

DPnew <- function (object, assay = NULL, features, cols = c("lightgrey", 
          ...
          scale = TRUE, scale.by = "radius", scale.min = NA, scale.max = NA) 
{
  ...
  data.plot <- lapply(X = unique(x = data.features$id), FUN = function(ident) {
    ...
    avg.exp <- apply(X = data.use, MARGIN = 2, FUN = function(x) {
      return(mean(x = exp(x = x))) # mean(exponential(x) - 1).
    })
  ...
  # reverse average expression
  avg.exp.scaled <- rev(avg.exp.scaled)
  ...
  if (split.colors) {
    ...
    print("gradient")
    plot <- plot + scale_color_gradient(low = cols[2], high = cols[1])
  }
  if (!split.colors) {
