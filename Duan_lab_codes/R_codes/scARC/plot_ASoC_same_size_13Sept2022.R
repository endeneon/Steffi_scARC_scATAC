...
for (i in 1:length(input_file_list)) {
  ...
  output_file_full_name <-
    str_remove_all(string = output_file_full_name,
                   pattern = '_same_target_reads_median_w_upsampling_12Sept2022')
  ...
  df_to_plot <- df_to_plot[df_to_plot$NCALLED > 2, ]
  df_to_plot <- df_to_plot[df_to_plot$NCALLED < 10, ]
...
sum(df_to_plot_output$FDR < 0.05 & 
  (df_to_plot_output$REF_C / df_to_plot_output$DP < 0.5))
