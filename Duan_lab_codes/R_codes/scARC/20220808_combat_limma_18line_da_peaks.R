...
assignTimexlineIdents <- function(obj) {
  cellines <- sort(unique(obj$cell.line.ident))
  times <- sort(unique(obj$time.ident))
  obj$timexline.ident <- NA
  for (l in cellines) {
    for (t in times) {
      id <- paste(l, t, sep = "-")
      print(id)
      obj$timexline.ident[obj$cell.line.ident == l &
                            obj$time.ident == t] <- id
    }
  }
  return(obj)
}
GABA <- assignTimexlineIdents(GABA)
nmglut <- assignTimexlineIdents(nmglut)
npglut <- assignTimexlineIdents(npglut)
unique(npglut$timexline.ident)
...
batch = c()
for (i in lts) {
  batch <- c(batch, unique(covar_18line$group[covar_18line$cell_line == str_extract(i, "CD_[0-9][0-9]")]))
}
batch
length(batch) #54
ncol(GABA_mat) #54
...
cnames <- c("1v0 up", "1v0 down", "6v0 up", "6v0 down", "6v1 up", "6v1 down")
...
# summary table ####
da_counts <- array(dim = c(3, 6), dimnames = list(types, cnames)) # 3 cell types are rows, 1/6 up/down are cols
for (i in 1:3){                                                               
  ...
  res_6v0 <- topTable(fit_all[[i]], coef = "sixvszero", p.value = 0.05, 
                      sort.by = "P", number = Inf)
  res_6v1 <- topTable(fit_all[[i]], coef = "sixvsone", p.value = 0.05, 
                      sort.by = "P", number = Inf)
  ...
  da_counts[i, 5] <- sum(res_6v1$logFC > 0)
  da_counts[i, 6] <- sum(res_6v1$logFC < 0)
}
...
da_peaks_by_time_list_6v1 <- vector("list", 3)
for (i in 1:length(da_peaks_by_time_list_1v0)) {
  ...
  da_peaks_by_time_list_6v1[[i]] <- topTable(fit_all[[i]], coef = "sixvsone", 
                                             p.value = Inf, sort.by = "P", 
                                             number = Inf)
}
...
lists <- list(da_peaks_by_time_list_1v0, da_peaks_by_time_list_6v0, da_peaks_by_time_list_6v1)
times <- c("1v0", "6v0", "6v1")
types
#setwd("/nvmefs/scARC_Duan_018/Duan_project_024_ATAC/da_peaks_limma")
