...
# input file ####
inputFiles <- "~/NVME/scARC_Duan_018/GRCh38_aggregation/sample_2_8_aggr_11Oct2021/outs/atac_fragments.tsv.gz"
...
proj$time.ident <- "NA"
for (t in times) {
  obj <- subset(obj_complete, time.ident == t)
...
for (t in types) {
  if (t == "NEFM+/CUX2- glut") {
    alt = "npglut"
  } else if (t == "NEFM-/CUX2+ glut") {
    alt = "nmglut"
  } else {
    alt = "GABA"
  }
  print(alt)
  obj <- subset(obj_complete, fine.cell.type == t)
  proj$cell.type[str_remove(proj$cellNames, "aggr#") %in% obj@assays$ATAC@counts@Dimnames[[2]]] <- alt
}
 unique(proj$cell.type)
 # 3. add cluster
 atac.cl <- unique(obj_complete$seurat_clusters)
 proj$ATAC.cluster.ident <- "NA"
 for (c in atac.cl) {
   print(c)
   obj <- subset(obj_complete, seurat_clusters == c)
 ...
 # 4. add group
 groups <- unique(obj_complete$time.group.ident)
 proj_sub$group.ident <- "NA"
 for (g in groups) {
   print(g)
   obj <- subset(obj_complete, time.group.ident == g)
   proj_sub$group.ident[str_remove(proj_sub$cellNames, "aggr#") %in% obj@assays$ATAC@counts@Dimnames[[2]]] <- g
 }
unique(proj_sub$group.ident)

# get gene matrix ####
idxType <- BiocGenerics::which(proj$cell.type != "NA")
cells <- proj$cellNames[idxType]
proj[cells]
obj_complete$broad.cell.type[is.na(obj_complete$broad.cell.type)] <- "NA"
sum(obj_complete$broad.cell.type %in% c("GABA", "glut"))
proj_sub <- proj[cells]
