# Chuxuan Li 03/20/2023
# Use MAST to do single cell DE analysis on CIRM cell lines that has PRS data
...
load("~/NVME/scARC_Duan_018/Duan_project_029_RNA/029_covariates_full_df.RData")
PRS_df <- read_excel("CIRM control iPSC lines_40_Duan_status_12Dec2022.xlsx", 
                     range = "D1:E53")

# assign covariates to single cells based on cell line ident ####
obj <- subset(integrated_labeled, cell.type != "unidentified")
unique(obj$cell.type)
rm(integrated_labeled)
lines <- PRS_df$`Catalog ID`[!is.na(PRS_df$`Polygenic SZ risk score`)]
obj <- subset(obj, cell.line.ident %in% lines)
lines <- unique(obj$cell.line.ident)
covar_df <- covar_df[covar_df$cell_line %in% lines, ]
...
for (l in lines) {
  print(l)
  age <- unique(covar_df$age[covar_df$cell_line == l])
  cat("\nage: ", age)
  obj$age[obj$cell.line.ident == l] <- age
  sex <- unique(covar_df$sex[covar_df$cell_line == l])
  cat("\nsex: ", sex)
    ...
    for (l in lines) {
      if (y == "GABA") {
        obj$cell.type.fraction[obj$cell.type == y & obj$time.ident == i & obj$cell.line.ident == l] <-
          covar_df$GABA_fraction[covar_df$time == i & covar_df$cell_line == l]
      } else if (y == "nmglut") {
        obj$cell.type.fraction[obj$cell.type == y & obj$time.ident == i & obj$cell.line.ident == l] <-
          covar_df$nmglut_fraction[covar_df$time == i & covar_df$cell_line == l]
      } else if (y == "npglut") {
        obj$cell.type.fraction[obj$cell.type == y & obj$time.ident == i & obj$cell.line.ident == l] <-
          covar_df$npglut_fraction[covar_df$time == i & covar_df$cell_line == l]
      }
    }
  }
}
unique(obj$cell.type.fraction)
save(obj, file = "obj_with_029_PRS_for_20230330_MAST_single_cell_DE.RData")

GABA <- subset(obj, cell.type == "GABA")
nmglut <- subset(obj, cell.type == "nmglut")
npglut <- subset(obj, cell.type == "npglut")
