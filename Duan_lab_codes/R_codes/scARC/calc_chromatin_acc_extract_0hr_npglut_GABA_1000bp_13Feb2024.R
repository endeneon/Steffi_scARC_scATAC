...
## expand peaks by +/- 250 bp #####
## check if any peaks has start < 250
sum(df_master_peak_set$start < 250)
df_master_peak_set$start <-
  df_master_peak_set$start - 250
df_master_peak_set$end <-
  df_master_peak_set$end + 250
...
save(GRanges_all_peaks,
     file = "Granges_npglut_0hr_peaks_non_overlapping_1000bp.RData")
...
## 0 hr specific criteria: FDR < 0.05 in 0 hr and #####
## binomP > 0.05 in GABA
names(vcf_lst_all_SNPs)

## get npglut 0 hr specific SNPs ####
npglut_0hr_ASoC_SNPs <-
  vcf_lst_all_SNPs[['0hr_npglut']][vcf_lst_all_SNPs[['0hr_npglut']]$FDR < 0.05, ]
GABA_0hr_pBinom_lessthan_005_SNPs <-
  vcf_lst_all_SNPs[['0hr_GABA']][vcf_lst_all_SNPs[['0hr_GABA']]$pVal < 0.05, ]

npglut_0hr_specific_snps <-
  npglut_0hr_ASoC_SNPs[!(npglut_0hr_ASoC_SNPs$ID %in% GABA_0hr_pBinom_lessthan_005_SNPs$ID), ]
npglut_0hr_specific_snps <-
  npglut_0hr_specific_snps[!duplicated(npglut_0hr_specific_snps$ID), ]

npglut_0hr_specific_interval <-
  npglut_0hr_specific_snps[, c(1, 2, 2, 3, 15)]
npglut_0hr_specific_interval$POS.1 <-
  as.numeric(npglut_0hr_specific_interval$POS)
...
peaks_flanking_npglut_0hr_SNPs <-
  GRanges_all_peaks[GenomicRanges::nearest(x = npglut_0hr_specific_interval,
                                           subject = GRanges_all_peaks,
                                           ignore.strand = T,
                                           select = "arbitrary")]
peaks_flanking_npglut_0hr_SNPs_distance <-
  GenomicRanges::distanceToNearest(x = npglut_0hr_specific_interval,
                                   subject = peaks_flanking_npglut_0hr_SNPs,
                                   ignore.strand = T)
sum(peaks_flanking_npglut_0hr_SNPs_distance@elementMetadata$distance == 0)
# hist(peaks_flanking_npglut_1hr_SNPs_distance@elementMetadata$distance)

peaks_flanking_npglut_0hr_SNPs <-
  peaks_flanking_npglut_0hr_SNPs[peaks_flanking_npglut_0hr_SNPs_distance@elementMetadata$distance == 0]
peaks_flanking_npglut_0hr_SNPs <-
  subsetByOverlaps(x = peaks_flanking_npglut_0hr_SNPs,
                   ranges = blacklist_hg38_unified,
                   invert = T)
peaks_flanking_npglut_0hr_SNPs <-
  peaks_flanking_npglut_0hr_SNPs[!duplicated(peaks_flanking_npglut_0hr_SNPs@elementMetadata@listData$peak_ID)]
save(peaks_flanking_npglut_0hr_SNPs,
     file = "peaks_inside_npglut_0hr_vs_GABA_SNPs_1000bp.RData")
