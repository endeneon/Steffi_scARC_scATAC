...
# need to subset cells: npglut-1hr
DefaultAssay(multiomic_obj) <- "ATAC"
unique(multiomic_obj$timextype.ident)
table(multiomic_obj$timextype.ident)

npglut_1hr_cells <-
  colnames(multiomic_obj)[multiomic_obj$timextype.ident %in% "npglut_1hr"]

npglut_1hr_peaks <-
  peaks_uncombined[[1]]
# confirm peak identity
unique(npglut_1hr_peaks$ident)

# remove peaks on nonstandard chromosomes and in genomic blacklist regions
npglut_1hr_peaks <-
  keepStandardChromosomes(npglut_1hr_peaks,
                          pruning.mode = "coarse")
npglut_1hr_peaks <-
  subsetByOverlaps(x = npglut_1hr_peaks,
                   ranges = blacklist_hg38_unified,
                   invert = TRUE)
unique(npglut_1hr_peaks@seqnames)
npglut_1hr_peaks_autosomes <-
  npglut_1hr_peaks %>%
  plyranges::filter(!(seqnames %in% c("chrX",
                                      "chrY")),
                    .preserve = T)
unique(npglut_1hr_peaks_autosomes@seqnames)
...
counts_raw <-
  FeatureMatrix(fragments = fragments_raw,
                features = npglut_1hr_peaks_autosomes,
                cells = npglut_1hr_cells,
                process_n = 5000,
                verbose = T)
# saveRDS(counts_raw,
...
assay_npglut_1hr <-
  CreateChromatinAssay(counts = counts_raw,
                       ...
                       annotation = ens_use)
seurat_npglut_1hr <-
  CreateSeuratObject(counts = assay_npglut_1hr,
                     assay = "peaks")

master_metadata_table <-
  multiomic_obj@meta.data
npglut_1hr_cells <-
  colnames(seurat_npglut_1hr)

seurat_metadata_table <-
  master_metadata_table[rownames(master_metadata_table) %in% npglut_1hr_cells, ]

seurat_npglut_1hr@meta.data <-
  seurat_metadata_table
...
### cut 0hr
covar_table_1hr <-
  covar_table_final[covar_table_final$time %in% "1hr", ]
covar_table_1hr <-
  covar_table_1hr[covar_table_1hr$cell_line %in% unique(seurat_npglut_1hr$cell.line.ident), ]

# seurat_npglut_1hr <-
#   seurat_npglut_1hr[, seurat_npglut_1hr$cell.line.ident %in% unique(seurat_npglut_1hr$cell_line)]

## Run TFIDF ####
seurat_npglut_1hr <-
  RunTFIDF(seurat_npglut_1hr,
           verbose = T)
seurat_npglut_1hr <-
  FindTopFeatures(seurat_npglut_1hr,
                  min.cutoff = "q5",
                  verbose = T)
seurat_npglut_1hr <-
  RunSVD(seurat_npglut_1hr,
         verbose = T)
seurat_npglut_1hr <-
  RunUMAP(seurat_npglut_1hr,
          reduction = "lsi",
          dims = 2:30)

## DA Analysis ####
head(seurat_npglut_1hr)
## assemble identity table
covar_table_1hr_short <-
  covar_table_1hr[ , c("cell_line",
                       "aff")]
seurat_indiv_table <-
  data.frame(barcode = colnames(seurat_npglut_1hr),
             cell_line = seurat_npglut_1hr$cell.line.ident)
seurat_indiv_table <-
  merge(x = seurat_indiv_table,
        y = covar_table_1hr_short,
        by = "cell_line",
        all.x = T)
sum(is.na(seurat_indiv_table$aff))
## assign aff identity to all cells
head(colnames(seurat_npglut_1hr))
head(seurat_indiv_table$barcode)

seurat_indiv_table <-
  seurat_indiv_table[match(x = colnames(seurat_npglut_1hr),
                           table = seurat_indiv_table$barcode), ]
head(seurat_indiv_table$barcode)
seurat_npglut_1hr$aff <-
  seurat_indiv_table$aff
unique(seurat_npglut_1hr$aff)

Idents(seurat_npglut_1hr) <- "aff"

da_seurat_peaks <-
  FindMarkers(object = seurat_npglut_1hr,
              ident.1 = "case",
              ...
              random.seed = 42,
              verbose = T)
save.image("DA_seurat_peaks_npglut_1hr_25Jan2023_w_results.RData")
