...
load("npglut_hr6_peaks_count_FeatureMatrix_76_lines_12Feb2024.RData")
...
Annotation(Seurat_npglut[['hr6_npglut_ATAC']]) <- hg38_annot
...
## confirm there is only one cell type in the opject
  ...
  for (j in 1:length(levels(seurat_2_count$library.ident))) {
    ...
    if (!(j %in% c(6))) {
      ...
      raw_rowcounts_matrix[, j] <-
        unlist(rowSums(subsetted_count_bcmatrix@assays$hr6_npglut_ATAC@counts))
    }
...
for (i in 1:length(levels(subsetted_seurat_npglut_cellss_only$time.ident))) {
  ...
  raw_count_post_combat <-
    raw_count_post_combat[, (colSums(raw_count_post_combat) > 80000)]
...
## 6vs0 hr, note use [[3]] ####
df_2_plot <-
  data.frame(x = rowMeans(SNP_flanking_peak_manual_calc[[3]],
                          na.rm = T),
...
ggplot(df_2_plot,
  ...
  # xlab("Accessibility in 6 hr npglut (log2 ATAC-seq peak RPKM)") +
  # ylab("Accessibility in 0 hr npglut (log2 ATAC-seq peak RPKM)") +
  scale_x_continuous(name = "Accessibility in 6 hr npglut (log2 ATAC-seq peak RPKM)",
                     expand = c(0, 0),
                     breaks = seq(0, 12, 2), 
                     limits = c(0, 12.5)) +
