# Chuxuan Li 02/20/2023
# Correlate the p-value and logFC of 029 and 024 limma results
...
setwd("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/limma_DE_results")
paths_029 <- list.files(path = "./unfiltered_by_padj", 
                        pattern = "[1|6]v0", full.names = T)
res_029 <- vector("list", length(paths_029))
for (i in 1:length(paths_029)) {
  res_029[[i]] <- as.data.frame(read_csv(paths_029[i]))[,2:8]
}
names(res_029) <- str_replace(str_extract(paths_029, "[A-Za-z]+_res_[1|6]v0"), "_res_", " ")
...
gene_counts_table <- matrix(nrow = 3, ncol = 3, 
                            dimnames = list(c("024", "029", "shared"), 
                                            str_remove(names(res_024)[c(1,3,5)], " 1v0")))

for (i in 1:length(res_024)) {
  genes[[i]] <- intersect(res_024[[i]]$gene, res_029[[i]]$gene)
  if (i %in% c(1,3,5)) {
    gene_counts_table[1, i %/% 2 + 1] <- length(res_024[[i]]$gene)
    gene_counts_table[2, i %/% 2 + 1] <- length(res_029[[i]]$gene)
    gene_counts_table[3, i %/% 2 + 1] <- length(genes[[i]])
  }
  res_024_new[[i]] <- res_024[[i]][res_024[[i]]$gene %in% genes[[i]], ]
  res_029_new[[i]] <- res_029[[i]][res_029[[i]]$gene %in% genes[[i]], ]
  
  res_024_new[[i]]$neglogp <- (-1) * log10(res_024_new[[i]]$P.Value)
  res_029_new[[i]]$neglogp <- (-1) * log10(res_029_new[[i]]$P.Value)
}
write.table(gene_counts_table, 
            file = "./024_029_limma_correlation_plots/shared_high_expression_gene_counts.csv",
            quote = F, sep = ",", row.names = T, col.names = T)

# all genes shared between 024 and 029 ####
for (i in 1:length(res_024_new)) {
  data <- res_024_new[[i]] %>% left_join(res_029_new[[i]], by = "gene")
  # correlation between p-values
  png(paste0("./024_029_limma_correlation_plots/all_shared_genes/", 
  ...
  p1 <- ggscatter(data = data,
                  ...
                  ylab = "029 limma p-value", xlim = c(0, 1), ylim = c(0, 1), 
                  title = paste0("Correlation between 024 and 029 ", names(res_024_new)[i], "\nlimma differential gene expression results - p value"))
  print(p1)
  dev.off()
  ...
  p2 <- ggscatter(data = data,
                  ...
                  ylab = "029 limma FDR", xlim = c(0, 1), ylim = c(0, 1), 
  ...
  p3 <- ggscatter(data = data,
                  ...
                  ylab = "029 limma logFC", #xlim = c(0, 1), ylim = c(0, 1), 
  ...
  p4 <- ggscatter(data = data,
                  ...
                  ylab = "029 limma -log10(p)",
                  title = paste0("Correlation between 024 and 029 ", names(res_024_new)[i], "\nlimma differential gene expression results - -log10p"))
  print(p4)
  dev.off()
}

# genes significant in either 024 or 029 ####
genes <- vector("list", length(res_024))
res_024_new <- res_024
res_029_new <- res_029
sig_counts_table <- matrix(nrow = 3, ncol = 6, 
                           dimnames = list(c("024", "029", "union"), names(res_024)))

for (i in 1:length(res_024)) {
  intersect_genes <- intersect(res_024[[i]]$gene, res_029[[i]]$gene)
  genes[[i]] <- union(res_024[[i]]$gene[res_024[[i]]$gene %in% intersect_genes & 
                                          res_024[[i]]$adj.P.Val < 0.05], 
                      res_029[[i]]$gene[res_029[[i]]$gene %in% intersect_genes &
                                          res_029[[i]]$adj.P.Val < 0.05])
  sig_counts_table[1, i] <- length(res_024[[i]]$gene[res_024[[i]]$gene %in% intersect_genes & 
                                                       res_024[[i]]$adj.P.Val < 0.05])
  sig_counts_table[2, i] <- length(res_029[[i]]$gene[res_029[[i]]$gene %in% intersect_genes & 
                                                       res_029[[i]]$adj.P.Val < 0.05])
...
write.table(sig_counts_table, file = "./024_029_limma_correlation_plots/shared_significant_gene_counts.csv",
            sep = ",", quote = F, row.names = T, col.names = T)

for (i in 1:length(res_024_new)) {
  data <- res_024_new[[i]] %>% left_join(res_029_new[[i]], by = "gene")
  # correlation between p-values
  png(paste0("./024_029_limma_correlation_plots/significant_genes_only/", 
