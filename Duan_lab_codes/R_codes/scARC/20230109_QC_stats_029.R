# Chuxuan Li 01/09/2023
# Summarize QC stats for 029 data
...
cell_counts <- read_csv("cell_counts_based_solely_on_GRCh38_mapped_data.csv")
...
cell_counts_df <- data.frame(library = rep(str_extract(string = h5list,
                             ...
                             type_of_count = rep(c("rat/rabbit", 
...
ggplot(data = cell_counts_df,
  ...
  ggtitle("Number of rat/rabbit cells,\nnon-rat/rabbit cells not mapped to human barcodes,\nand cells mapped to human barcodes per library")
...
ggplot(data = cell_counts_df,
  ...
  ylab("Percentage of ") +
  labs(fill = "Cell Types") +
  ggtitle("Percentage of rat/rabbit cells,\nnon-rat/rabbit cells not mapped to human barcodes,\nand cells mapped to human barcodes per library")
...
load("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/GRCh38_mapped_removed_rat_cells_assigned_demux_bc_list.RData")
for (i in 1:length(libnames)){
  print(libnames[i])
...
write.table(df_human, 
            file = "./QC/no_rat_rabbit_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
write.table(df_human, 
            file = "./QC/mapped_to_demuxlet_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
df_human$library <- factor(df_human$library, levels = rev(libnames))
plotCellCountsRectangles(df_human)
...
write.table(df_qc, 
            file = "./QC/QC_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
load("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/integrated_obj_nfeature_8000.RData")
...
write.table(cell_counts_reads_df, 
            file = "./QC/cell_counts_UMI_counts_per_line.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
