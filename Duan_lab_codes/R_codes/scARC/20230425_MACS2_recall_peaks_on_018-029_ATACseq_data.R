# Chuxuan Li 04/25/2023
# Use MACS2 to recall the peaks from 018-029 combined ATACseq data, 
#then count the fragments and repeat normalizing, clustering, plotting
...
load("./018-029_multiomic_obj_clustered_added_metadata.RData")
...
save(peaks, file = "018-029_macs2_called_new_peaks_combined.RData")
...
save(peaks_uncombined, file = "018-029_macs2_called_new_peaks_sep_by_timextype.RData")
...
write.table(df, 
            file = "./MACS2_called_new_peaks/018-029_union_peak_set.bed", 
            quote = F, 
            sep = "\t", 
...
for (i in 1:length(peaks_uncombined)){
  ...
  file_name <- paste0("./MACS2_called_new_peaks/018-029_sep_peaks_called_in_",
...
save(macs2_counts, file = "MACS2_recalled_018-029_peaks_FeatureMatrix_output.RData")

fragpath <- "./018_to_029_combined/outs/atac_fragments.tsv.gz"
...
save(multiomic_obj, file = "018-029_multiomic_obj_added_MACS2_new_peaks.RData")
...
DimPlot(object = multiomic_obj_new, label = TRUE, group.by = "RNA.cell.type") + 
  ...
  theme(text = element_text(size = 10))
DimPlot(object = multiomic_obj_new, label = TRUE, group.by = "RNA.fine.cell.type") + 
  NoLegend() +
  ggtitle("ATACseq data w/ new peak set projected by detailed RNAseq cell types") +
  theme(text = element_text(size = 10))
