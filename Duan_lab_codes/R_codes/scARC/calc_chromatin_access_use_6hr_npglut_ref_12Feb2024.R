...
# load data #####
load("~/NVME/scARC_Duan_018/018-029_combined_analysis/npglut_hr6_count_FeatureMatrix_76_lines_08Feb2024.RData")
...
npglut_hr6_peaks_count <-
  npglut_hr6_peaks_count[!duplicated(rownames(npglut_hr6_peaks_count)), ]
signac_npglut_hr1_peaks <-
  CreateChromatinAssay(counts = npglut_hr6_peaks_count,
                       sep = c("-", "-"),
                       genome = "hg38",
                       fragments = ATAC_fragment)
Seurat_npglut_hr6_peaks <-
  CreateSeuratObject(counts = signac_npglut_hr1_peaks,
                     assay = "ATAC",
                     meta.data = metadata_multiomic_obj)

DefaultAssay(Seurat_npglut_hr6_peaks)
hg38_annot <-
...
genome(hg38_annot) <-
  'hg38'
Annotation(Seurat_npglut_hr6_peaks[['ATAC']]) <- hg38_annot

Seurat_npglut_hr6_peaks <-
  FindTopFeatures(Seurat_npglut_hr6_peaks,
                  min.cutoff = 10,
                  verbose = T)
Seurat_npglut_hr6_peaks <-
  RunTFIDF(Seurat_npglut_hr6_peaks,
           verbose = T)
Seurat_npglut_hr6_peaks <-
  RunSVD(Seurat_npglut_hr6_peaks,
         verbose = T)
Seurat_npglut_hr6_peaks <-
  RunUMAP(Seurat_npglut_hr6_peaks,
          reduction = 'lsi',
          dims = 2:30,
          reduction.name = 'umap.atac',
          seed.use = 42)

DimPlot(Seurat_npglut_hr6_peaks,
        reduction = 'umap.atac') +
  NoLegend()

unique(Seurat_npglut_hr6_peaks$RNA.cell.type)
subsetted_seurat_npglut_cellss_only <-
  Seurat_npglut_hr6_peaks[, Seurat_npglut_hr6_peaks$RNA.cell.type == "npglut"]
...
## 6vs0 hr ####
df_2_plot <-
  data.frame(x = rowMeans(SNP_flanking_peak_cpm[[3]],
                          na.rm = T),
...
ggplot(df_2_plot,
  ...
  xlab("Accessibility in 6 hr npglut (log2 ATAC-seq peak CPM)") +
  ...
  geom_text(x = 1,
            ...
            label = paste0(signif(perc_comparable * 100,
                           ...
                           '%')) +
  theme_minimal() +
  ggtitle(paste("npglut 6hr-specific ASoC SNP-flanking peaks",
                "n = 10670",
                sep = "\n"))
