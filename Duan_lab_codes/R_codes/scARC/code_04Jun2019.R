...
SNP_unique_table[32, ] <- c("cell_type_specific", "Enh_all", sum(SNP_unique_table$length[4:9]))
...
annotations_features_count[32, ] <- c(sum(as.numeric(annotations_features_count$total_features[4:9])),
                                      "z_all_Enh_Added", "Enh_all")
...
for(i in 1:nrow(SNP_unique_table)) {
  ...
  SNP_unique_table$GREATenrichment[i] <- (0-log10(binom.test(x = as.numeric(SNP_unique_table$cell_type[i]),
                                                             n = 4411, #357, #4411, #357# #SNP_unique_table$n_total_features_length[i],
                                                             p = as.numeric(as.character(SNP_unique_table$kpi_SNP_count[i]))/3234830000,
                                                             alternative = "greater")$p.value))
  # binomP <- mpfr(0, precBits = 3500)
