...
filterByCpm <- function(df1, df2, cutoff, proportion = 1) {
  cpm1 <- cpm(df1)
  cpm2 <- cpm(df2)
...
tables <- list(covar_table_0hr, covar_table_1hr, covar_table_6hr)
names(tables) <- c("covar_table_0hr", "covar_table_1hr", "covar_table_6hr")
...
designs <- list(designs_0hr, designs_1hr, designs_6hr)
names(designs) <- c("designs_0hr", "designs_1hr", "designs_6hr")

celllines <- unique(str_remove(colnames(adj_mat_lst[[1]]), "_[0|1|6]hr$"))
for (i in 1:length(tables)) {
  ...
  for (j in 1:length(adj_mat_lst)) {
    ...
    designs[[i]][[j]] <- model.matrix(~0 + batch + age_sq + sex + f, 
                                 data = table)
  }
...
df_list <- list(df_0hr, df_1hr, df_6hr)
names(df_list) <- c("df_0hr", "df_1hr", "df_6hr")
...
for (i in 1:length(fitted_1v0)) {
  resid_1v0[[i]] <- list(residuals.MArrayLM(object = fitted_1v0[[i]][[1]], voomed_1v0[[i]][[1]]),
                    residuals.MArrayLM(object = fitted_1v0[[i]][[2]], voomed_1v0[[i]][[2]]))
  resid_6v0[[i]] <- list(residuals.MArrayLM(object = fitted_6v0[[i]][[1]], voomed_6v0[[i]][[1]]),
                         residuals.MArrayLM(object = fitted_6v0[[i]][[2]], voomed_6v0[[i]][[2]]))
}
...
save(resid_diff_1v0, resid_diff_6v0, file = "cpm_filtered_residual_differences_1v0_6v0.RData")
...
for (i in 1:length(designs_1v0)) {
  designs_1v0[[i]] <- model.matrix(~0 + aff, data = covar_table_1hr)
  designs_6v0[[i]] <- model.matrix(~0 + aff, data = covar_table_0hr)
}
...
for (i in 1:length(resid_1v0)) {
  cvc_1v0[[i]] <- apply(X = resid_diff_1v0[[i]], MARGIN = 1, FUN = lmfeeder)
  cvc_6v0[[i]] <- apply(X = resid_diff_6v0[[i]], MARGIN = 1, FUN = lmfeeder)
}
...
for (i in 1:length(cvc_6v0)) {
  for(j in 1:length(cvc_6v0[[i]])) {
    if (j == 1) {
      mat1 <- summary(cvc_6v0[[i]][[j]])$coefficients["affcase", ]
    } else {
      mat1 <- rbind(mat1, summary(cvc_6v0[[i]][[j]])$coefficients["affcase", ])
    }
  }
...
write.table(deg_counts, 
            file = "./018-029_diff_response_filter_by_cpm_casevcontrol_lm_results/DRG_counts_filter_by_pval.csv", 
            sep = ",", quote = F)

for (i in 1:length(betas_1v0)) {
  df <- betas_1v0[[i]][betas_1v0[[i]]$p.adj < 0.05 & betas_1v0[[i]]$beta > 0, ]
  filename <- 
    paste0("./018-029_diff_response_filter_by_cpm_casevcontrol_lm_results/filtered_by_padj/",
  ...
  filename <- 
    paste0("./018-029_diff_response_filter_by_cpm_casevcontrol_lm_results/filtered_by_pvalue/",
