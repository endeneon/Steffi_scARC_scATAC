# Chuxuan li 08/17/2022
# separate the time points and perfrom DE analysis between case and control in 
#each of the time points, combining 5, 18, and 20 line data.
# protocol same with 18Jul2022_pseudobulk_DE_by_limma_batch_in_design_and_voom.R,
#first generate raw count matrix, then apply combat, then use limma
...
for (i in 1:length(celltypes)) {
  t <- celltypes[i]
  print(t)
  sub_5 <- subset(RNA_5line, cell.type == t)
  sub_18 <- subset(RNA_18line, cell.type == t)
  sub_20 <- subset(RNA_20line, cell.type == t)
  subobjs <- list(sub_5, sub_18, sub_20)
  zero_objs <- vector("list", 3)
  one_objs <- vector("list", 3)
  six_objs <- vector("list", 3)
  for (j in 1:3) {
  ...
  names(zero_objs) <- c("5", "18", "20")
  names(one_objs) <- c("5", "18", "20")
  names(six_objs) <- c("5", "18", "20")
...
save(fits, file = "case_v_control_limma_fits_5+18+20line.RData")

# plot results ####
deg_df <- NULL
for (i in 1:length(fits)) {
  print(names(adj_mat_lst)[i])
