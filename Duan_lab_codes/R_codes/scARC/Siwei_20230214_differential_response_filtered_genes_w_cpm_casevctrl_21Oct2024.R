# Siwei 21 Oct 2024
# test differential responses by aff
...
# make limma objects and filter by cpm (1) #####
{
...
# load("cpm_filtered_residual_differences_1v0_6v0_dif_sex.RData")
...
# make a more complex design matrix
for (i in 1:length(designs_1v0)) {
  designs_1v0[[i]] <- 
    model.matrix(~ 0 + aff +
                   sex + age, 
...
# ! contrast was made by case vs ctrl !! ####
contrastOnSubtractedVal <- 
  function(df, design) {
  fit <- lmFit(df, design)
  contr.matrix <- makeContrasts(
    CasevsCtrl = affcase - affcontrol,
...
factor_aff <- 
  factor(covar_table_final$aff[covar_table_final$time == "0hr"], 
         levels = c("case", "control"))
times <- sort(unique(covar_table_final$time))

lmfeeder <- function(row) {
  return(lm(row ~ factor_aff))
}
...
for (i in 1:length(cvc_1v0)) {
  for (j in 1:length(cvc_1v0[[i]])) {
    if (j == 1) {
      mat1 <- summary(cvc_1v0[[i]][[j]])$coefficients["factor_affcontrol", ]
    } else {
      mat1 <- rbind(mat1, summary(cvc_1v0[[i]][[j]])$coefficients["factor_affcontrol", ])
    }
  }
...
for (i in 1:length(cvc_6v0)) {
  for (j in 1:length(cvc_6v0[[i]])) {
    if (j == 1) {
      mat1 <- summary(cvc_6v0[[i]][[j]])$coefficients["factor_affcontrol", ]
    } else {
      mat1 <- rbind(mat1, summary(cvc_6v0[[i]][[j]])$coefficients["factor_affcontrol", ])
    }
  }
...
output_dir <- "Siwei_casevsctrl_lm_results_21Oct2024"
...
write.table(deg_counts, 
            file = paste(".",
                         output_dir,
                         "Siwei_casevsctrl_lm_018-029_21Oct2024.txt",
                         sep = "/"), 
            sep = "\t", quote = F)
...
for (i in 1:length(betas_1v0)) {
  ...
  filename <-
    paste0("./",
           output_dir,
           "/",
           names(betas_1v0)[i], "_1v0response_all_genes_casevsctrl.txt")
  write.table(df, 
              file = filename, 
              quote = F, sep = "\t", 
              row.names = T, 
  ...
  filename <-
    paste0("./",
           output_dir,
           "/",
           names(betas_6v0)[i], "_6v0response_all_genes_casevsctrl.txt")
  write.table(df, 
              file = filename, 
              quote = F, sep = "\t", 
              row.names = T, 
