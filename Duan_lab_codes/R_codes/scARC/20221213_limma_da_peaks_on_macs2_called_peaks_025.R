# Chuxuan Li 12/13/2022
# Differential accessibility with limma on 025 data, based on 08Aug2022_combat_limma_18line_da_peaks.R
...
load("../Duan_project_025_RNA/Analysis_part2_GRCh38/025_covariates_full_df.RData")
load("/nvmefs/scARC_Duan_018/Duan_project_025_ATAC/ATAC_clean_obj_added_MACS2_peaks.RData")
...
ATAC_clean$timexline.ident <- NA
cellines <- sort(unique(ATAC_clean$cell.line.ident))
times <- sort(unique(ATAC_clean$time.ident))
for (l in cellines) {
  for (t in times) {
    id <- paste(l, t, sep = "-")
    print(id)
    ATAC_clean$timexline.ident[ATAC_clean$cell.line.ident == l &
                                 ATAC_clean$time.ident == t] <- id
  }
}
GABA <- subset(ATAC_clean, RNA.cell.type == "GABA")
nmglut <- subset(ATAC_clean, RNA.cell.type == "NEFM- glut")
npglut <- subset(ATAC_clean, RNA.cell.type == "NEFM+ glut")
...
batch = c()
for (i in lts) {
  batch <- c(batch, unique(covar_df$batch[covar_df$cell_line == str_extract(i, "CD_[0-9][0-9]")]))
}
batch
length(batch) #60
ncol(GABA_mat) #60
...
save(GABA_mat_adj, nmglut_mat_adj, npglut_mat_adj, file = "post-combat_025_ATAC_count_matrices.RData")
...
design_GABA <- model.matrix(~0 + time + batch + age + sex + aff + GABA_fraction, data = covar_df) 
y_GABA <- createDGE(GABA_mat_adj)
ind.keep <- filterByCpm(y_GABA, 1, 10)
sum(ind.keep) #433918 -> 155046
...
design_nmglut <- model.matrix(~0 + time + batch + age + sex + aff + nmglut_fraction, data = covar_df) 
y_nmglut <- createDGE(nmglut_mat_adj)
ind.keep <- filterByCpm(y_nmglut, 1, 10)
sum(ind.keep) #433918 -> 180840
...
design_npglut <- model.matrix(~0 + time + batch + age + sex + aff + npglut_fraction, data = covar_df)
y_npglut <- createDGE(npglut_mat_adj)
ind.keep <- filterByCpm(y_npglut, 1, 10)
sum(ind.keep) #433918 -> 200579
...
# output results ####
setwd("/nvmefs/scARC_Duan_018/Duan_project_025_ATAC/dapeaks_limma/")

fit_all <- list(fit_GABA_contr, fit_nmglut_contr, fit_npglut_contr)
save(fit_all, file = "025_limma_da_peaks_list_of_three_celltypes_contrast_fit_results.RData")
