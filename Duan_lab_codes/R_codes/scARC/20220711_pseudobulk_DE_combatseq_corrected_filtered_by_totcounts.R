...
batch <- rep(c(12, 11, 12, 12, 8, 8, 9, 9, 10, 8, 8, 9, 10, 10, 11, 11, 11, 10), each = 3) 
...
p$breaks[5]
keep <- rowSums(counts(dds_GABA)) >= 300
...
(p$counts[1] + p$counts[2] + p$counts[3] + p$counts[4] + p$counts[5])/sum(p$counts)
p$breaks[6]
keep <- rowSums(counts(dds_npglut)) >= 450
...
write.table(deg_counts, file = "./pseudobulk_DE/deg_summary_combat_by_batch_50pctgenes.csv", quote = F, sep = ",", row.names = T, col.names = T)
...
for (j in 1:length(reslists)) {
  ...
  for (i in 1:length(lst)){                                                               
    ...
    write.table(res, file = filename, quote = F, sep = ",", col.names = T, 
                row.names = F)
    filename <- paste0("./pseudobulk_DE/res_use_combatseq_mat/upreg_padj_filtered/", 
                       types[i], "_", resorder[j],
                       "_upregulated_significant.csv")
    print(filename)
    write.table(res[res$padj < 0.05 & res$log2FoldChange > 0, ],
                file = filename, quote = F, sep = ",", col.names = T, 
                row.names = F)
    filename <- paste0("./pseudobulk_DE/res_use_combatseq_mat/downreg_padj_filtered/", 
                       types[i], "_", resorder[j],
...
for (j in 1:length(reslists)) {
  ...
  for (i in 1:length(lst)){                                                               
    ...
    print(filename)
    write.table(rownames(res[res$padj < 0.05 & res$log2FoldChange > 0, ]),
...
gene_labeller <- as_labeller(c('BDNF' = "BDNF (excitatory)", 
                               ...
                               'VGF' = "VGF (shared)"))
ggplot(df_to_plot, aes(x = cell.type, y = log2FoldChange,
                       color = cell.type, fill = time, group = time,
  ...
  ggtitle("differential expression of late response genes - corrected by combat")
