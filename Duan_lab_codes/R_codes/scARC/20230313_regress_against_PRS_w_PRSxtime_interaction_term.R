...
#and added interaction term (PRS x time)
...
design_GABA <- model.matrix(~PRS + time + batch + age + sex + GABA_fraction + PRS:time, 
                            data = covar_table_PRS) # this finds genes respond differently to stimulation with the same PRS
colnames(design_GABA) # intercept = average log expr for 0hr cells
...
design_nmglut <- model.matrix(~PRS + time + batch + age + sex + nmglut_fraction + PRS:time, 
...
design_npglut <- model.matrix(~PRS + time + batch + age + sex + npglut_fraction + PRS:time, 
                              data = covar_table_PRS)
...
setwd("./sep_by_celltype_w_time_interaction_DE_results/")
types <- names(adj_mat_lst)
for (i in 1:3){
  res <- topTable(fit_all[[i]], coef = "PRS:time6hr", p.value = 0.05, sort.by = "P", number = Inf)
  res$gene <- rownames(res)
  
  filename <- paste0("./upregulated_significant/", types[i], "_PRS_6v0hr_upregulated_DEGs.csv")
  print(filename)
  write.table(res[res$logFC > 0, ], file = filename, quote = F, sep = ",", 
              row.names = F, col.names = T)
  filename <- paste0("./downregulated_significant/", types[i], "_PRS_6v0hr_downregulated_DEGs.csv")
  print(filename)
  write.table(res[res$logFC < 0, ], file = filename, quote = F, sep = ",", 
              row.names = F, col.names = T)  
  
  res <- topTable(fit_all[[i]], coef = "PRS:time6hr", p.value = Inf, sort.by = "P", number = Inf)
  res$gene <- rownames(res)
  
  filename <- paste0("./unfiltered_by_padj/", types[i], "_PRS_6v0hr_upregulated_DEGs.csv")
  print(filename)
  write.table(res[res$logFC > 0, ], file = filename, 
              quote = F, sep = ",", row.names = F, col.names = T)
  
  filename <- paste0("./unfiltered_by_padj/", types[i], "_PRS_6v0hr_downregulated_DEGs.csv")
  print(filename)
  write.table(res[res$logFC < 0, ], file = filename, 
              quote = F, sep = ",", row.names = F, col.names = T)
  
  filename <- paste0("./unfiltered_by_padj/", types[i], "_PRS_6v0hr_all_DEGs.csv")
  print(filename)
...
# summary table
cnames <- c("beta > 0 (1v0)", "beta < 0 (1v0)", "beta > 0 (6v0)", "beta < 0 (6v0)",
            "total passed filter")
deg_counts <- array(dim = c(3, 5), dimnames = list(types, cnames)) # 3 cell types are rows, 1/6 up/down are cols
for (i in 1:3){                                                               
  res_1v0 <- topTable(fit_all[[i]], coef = "PRS:time1hr", p.value = Inf, sort.by = "P", number = Inf)
  res_6v0 <- topTable(fit_all[[i]], coef = "PRS:time6hr", p.value = Inf, sort.by = "P", number = Inf)
