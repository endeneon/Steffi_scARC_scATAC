...
# Correlate the p-value and logFC of 025 and 024 DESeq2 results
...
paths_025 <- list.files(path = "./Analysis_part2_GRCh38/DESeq2_DE_results/full_no_filtering", 
                        pattern = "[1|6]v0", full.names = T)
res_025 <- vector("list", length(paths_025))
for (i in 1:length(paths_025)) {
  res_025[[i]] <- as.data.frame(read_csv(paths_025[i]))
}
names(res_025) <- str_extract(paths_025, "[A-Za-z]+_[1|6]v0")

paths_024 <- list.files(path = "../Duan_project_024_RNA/Analysis_v1_normalize_by_lib_then_integrate/pseudobulk_DE/res_use_combatseq_mat/filtered_by_basemean_only", 
                        pattern = "[1|6]v0", full.names = T)
res_024 <- vector("list", length(paths_024))
for (i in 1:length(paths_024)) {
  res_024[[i]] <- as.data.frame(read_csv(paths_024[i]))
}
names(res_024) <- str_extract(paths_024, "[A-Za-z]+_[1|6]v0")
...
for (i in 1:length(res_024)) {
  ...
  res_024_new[[i]]$neglogp <- (-1) * log10(res_024_new[[i]]$pvalue)
  res_025_new[[i]]$neglogp <- (-1) * log10(res_025_new[[i]]$pvalue)
}
write.table(gene_counts_table, 
            file = "./Analysis_part2_GRCh38/024_025_DESeq2_correlation_plots/shared_high_expression_gene_counts.csv",
...
for (i in 1:length(res_024_new)) {
  ...
  png(paste0("./Analysis_part2_GRCh38/024_025_DESeq2_correlation_plots/all_shared_genes/", 
             str_replace(names(res_024_new)[i], " ", "_"), "_all_gene_pval_corr.png"))
  p1 <- ggscatter(data = data,
                  x = "pvalue.x", y = "pvalue.y", size = 0.01, add = "reg.line", conf.int = T, 
                  cor.coef = T, cor.method = "pearson", xlab = "024 DESeq2 p-value",
                  ylab = "025 DESeq2 p-value", xlim = c(0, 1), ylim = c(0, 1), 
                  title = paste0("Correlation between 024 and 025 ", names(res_024_new)[i], "\nDESeq2 differential gene expression results - p value"))
  print(p1)
  dev.off()
  ...
  p2 <- ggscatter(data = data,
                  x = "padj.x", y = "padj.y", size = 0.01, add = "reg.line", conf.int = T, 
                  cor.coef = T, cor.method = "pearson", xlab = "024 DESeq2 FDR",
                  ylab = "025 DESeq2 FDR", xlim = c(0, 1), ylim = c(0, 1), 
                  title = paste0("Correlation between 024 and 025 ", names(res_024_new)[i], "\nDESeq2 differential gene expression results - FDR"))
  print(p2)
  dev.off()
  ...
  p3 <- ggscatter(data = data,
                  x = "log2FoldChange.x", y = "log2FoldChange.y", size = 0.01, add = "reg.line", conf.int = T, 
                  cor.coef = T, cor.method = "pearson", xlab = "024 DESeq2 logFC",
                  ylab = "025 DESeq2 logFC", #xlim = c(0, 1), ylim = c(0, 1), 
                  title = paste0("Correlation between 024 and 025 ", names(res_024_new)[i], "\nDESeq2 differential gene expression results - logFC"))
  print(p3)
  dev.off()
  ...
  p4 <- ggscatter(data = data,
                  ...
                  cor.coef = T, cor.method = "pearson", xlab = "024 DESeq2 -log10(p)",
                  ylab = "025 DESeq2 -log10(p)",
                  title = paste0("Correlation between 024 and 025 ", names(res_024_new)[i], "\nDESeq2 differential gene expression results - -log10p"))
  print(p4)
  dev.off()
}
...
write.table(sig_counts_table, file = "./Analysis_part2_GRCh38/024_025_DESeq2_correlation_plots/shared_significant_gene_counts.csv",
            sep = ",", quote = F, row.names = T, col.names = T)

for (i in 1:length(res_024_new)) {
  data <- res_024_new[[i]] %>% left_join(res_025_new[[i]], by = "gene")
  # correlation between p-values
  png(paste0("./Analysis_part2_GRCh38/024_025_DESeq2_correlation_plots/significant_genes_only/", 
             str_replace(names(res_024_new)[i], " ", "_"), "_sig_gene_pval_corr.png"))
  p1 <- ggscatter(data = data,
                  x = "pvalue.x", y = "pvalue.y", size = 0.01, add = "reg.line", conf.int = T, 
                  cor.coef = T, cor.method = "pearson", xlab = "024 DESeq2 p-value",
                  ylab = "025 DESeq2 p-value", xlim = c(0, 1), ylim = c(0, 1), 
                  title = paste0("Correlation between 024 and 025 ", names(res_024_new)[i], "\nDESeq2 FDR<0.05 genes - p value"))
  print(p1)
  dev.off()
  # correlation between FDR
  png(paste0("./Analysis_part2_GRCh38/024_025_DESeq2_correlation_plots/significant_genes_only/", 
