...
# Summarize QC stats for 029 40201 data
...
cell_counts <- read_csv("./40201_plots/cell_counts_40201.csv")
...
for (i in 1:length(cell_counts$library)){
  cell_counts_df$count[i * 3] <- cell_counts$mapped_to_human_barcodes[cell_counts$library == cell_counts_df$library[i * 3]]
  cell_counts_df$count[i * 3 - 1] <- 
    cell_counts$not_mapped_to_human_barcodes[cell_counts$library == cell_counts_df$library[i * 3]]
  cell_counts_df$count[i * 3 - 2] <- cell_counts$rat_or_rabbit[cell_counts$library == cell_counts_df$library[i * 3]]
}
...
# raw
load("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/GRCh38_mapped_raw_list_40201_only.RData")
...
write.table(df_raw, 
            file = "./40201_plots/40201_QC/raw_cell_counts_UMI_counts_per_lib.csv",
...
load("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/GRCh38_mapped_removed_rat_cells_assigned_demux_bc_list_40201.RData")
for (i in 1:length(libnames)){
  print(libnames[i])
...
write.table(df_human, 
            file = "./40201_plots/40201_QC/no_rat_rabbit_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
write.table(df_demux, 
            file = "./40201_plots/40201_QC/mapped_to_demuxlet_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
# after QC
load("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/GRCh38_mapped_after_QC_list_40201.RData")
for (i in 1:length(libnames)){
  print(libnames[i])
...
write.table(df_qc, 
            file = "./40201_plots/40201_QC/QC_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
load("/nvmefs/scARC_Duan_018/Duan_project_029_RNA/integrated_obj_40201.RData")
...
write.table(cell_counts_reads_df, 
            file = "./40201_plots/40201_QC/cell_counts_UMI_counts_per_line.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
for (i in 1:3){
  ...
  png(filename = paste0("./40201_plots/40201_QC/full_cellular_compos_", times[i], ".png"),
...
for (i in 1:3){
  df_to_plot <- time_type_sum[[i]]
  png(filename = paste0("./40201_plots/40201_QC/general_cellular_compos_", times[i], ".png"),
      width = 300, height = 750)
  p <- ggplot(data = df_to_plot,
              aes(x = cell.line, 
                  y = counts, 
