...
input_file_list <-
  list.files(path = "SNP_downsized_22Aug2022/", 
             pattern = "R\\.txt$",
             full.names = T)
output_path <- "SNP_18_lines_2_annot/"

input_file_list <-
  list.files(path = "bowtie2_test", 
             pattern = "GABA_0hr.*R\\.txt$",
             full.names = T)
output_path <- "bowtie2_test"
...
for (i in 1:length(input_file_list)) {
  ...
  df_to_plot <- df_to_plot[df_to_plot$DP > 9, ]
  ...
  # fig_to_plot <- 
    ...
    ggtitle(paste(output_file_full_name, ", DP >= 12, 5 lines\n",
    ...
    sum(df_to_plot$REF_C / df_to_plot$DP < 0.5) / length(df_to_plot$DP)
