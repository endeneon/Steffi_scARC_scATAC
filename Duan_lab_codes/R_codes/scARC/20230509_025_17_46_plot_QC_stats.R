...
# Summarize QC stats for 025 17&46 data
...
cell_counts <- read_csv("./Analysis_part2_GRCh38/plots_from_lib_17_46/cell_counts_based_solely_on_GRCh38_mapped_data_17_46.csv")
...
for (i in 1:length(cell_counts$library)){
  ...
  cell_counts_df$count[i * 3 - 2] <- cell_counts$rat[cell_counts$library == cell_counts_df$library[i * 3]]
}
...
df_human <- data.frame(library = libnames,
                       ...
                       n.reads = rep_len(0, length(libnames)),
                       reads.per.cell = rep_len(0, length(libnames)))
df_demux <- data.frame(library = libnames,
                       cell.counts = rep_len(0, length(libnames)),
                       n.reads = rep_len(0, length(libnames)),
                       reads.per.cell = rep_len(0, length(libnames)))
# raw
load("/nvmefs/scARC_Duan_018/Duan_project_025_RNA/Analysis_part2_GRCh38/GRCh38_mapped_raw_list_17_46.RData")
...
write.table(df_raw, 
            file = "./Analysis_part2_GRCh38/plots_from_lib_17_46/raw_cell_counts_UMI_counts_per_lib.csv",
...
load("/nvmefs/scARC_Duan_018/Duan_project_025_RNA/Analysis_part2_GRCh38/GRCh38_mapped_removed_rat_cells_assigned_demux_bc_list_17_46.RData")
for (i in 1:length(libnames)){
  print(libnames[i])
  df_human$cell.counts[i] <- length(noratlist[[i]]@assays$RNA@counts@Dimnames[[2]])
  df_human$n.reads[i] <- sum(noratlist[[i]]@assays$RNA@counts@x)
  df_human$reads.per.cell[i] <- 
    sum(noratlist[[i]]@assays$RNA@counts@x)/df_human$cell.counts[i]
}
write.table(df_human, 
            file = "./Analysis_part2_GRCh38/plots_from_lib_17_46/no_rat_rabbit_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
for (i in 1:length(libnames)){
  print(libnames[i])
  obj <- subset(noratlist[[i]], cell.line.ident != "unmatched")
  df_demux$cell.counts[i] <- length(obj@assays$RNA@counts@Dimnames[[2]])
  df_demux$n.reads[i] <- sum(obj@assays$RNA@counts@x)
  df_demux$reads.per.cell[i] <- 
    sum(obj@assays$RNA@counts@x)/df_demux$cell.counts[i]
}
write.table(df_demux, 
            file = "./Analysis_part2_GRCh38/plots_from_lib_17_46/mapped_to_demuxlet_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
df_demux$library <- factor(df_demux$library, levels = rev(libnames))
plotCellCountsRectangles(df_demux)
plotUMIBars(df_demux)
plotTotalUMIBars(df_demux)

# after QC
load("/nvmefs/scARC_Duan_018/Duan_project_025_RNA/Analysis_part2_GRCh38/GRCh38_mapped_after_QC_list_17_46.RData")
for (i in 1:length(libnames)){
  print(libnames[i])
...
write.table(df_qc, 
            file = "./Analysis_part2_GRCh38/plots_from_lib_17_46/QC_cell_counts_UMI_counts_per_lib.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
load("/nvmefs/scARC_Duan_018/Duan_project_025_RNA/Analysis_part2_GRCh38/integrated_obj_17_46.RData")
...
write.table(cell_counts_reads_df, 
            file = "./Analysis_part2_GRCh38/plots_from_lib_17_46/cell_counts_UMI_counts_per_line.csv",
            quote = F, sep = ",", row.names = F,
            col.names = T)
...
load("/nvmefs/scARC_Duan_018/Duan_project_025_RNA/Analysis_part2_GRCh38/integrated_clustered_obj_17_46.RData")
...
for (k in 1:length(times)){
  ...
  df$cell.type <- factor(df$cell.type, levels = c("GABA", "SEMA3E+ GABA", "SST+ GABA",
                                                  "NEFM+ glut", "NEFM- glut", "unknown neuron",
                                                  "immature neuron", "unknown"))
  time_type_sum[[k]] <- df
}

for (i in 1:3){
  df_to_plot <- time_type_sum[[i]]
  png(filename = paste0("./Analysis_part2_GRCh38/plots_from_lib_17_46/full_cellular_compos_", times[i], ".png"),
...
for (i in 1:3){
  df_to_plot <- time_type_sum[[i]]
  png(filename = paste0("./Analysis_part2_GRCh38/plots_from_lib_17_46/general_cellular_compos_", times[i], ".png"),
      width = 500, height = 750)
  p <- ggplot(data = df_to_plot,
              aes(x = cell.line, 
                  y = counts, 
