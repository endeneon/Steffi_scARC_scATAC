...
# Use limma to test stimulation DEG in 029 data
...
load("029_CIRM_metadata_df.RData")
...
lines <- str_extract(colnames(GABA_mat), "CW[0-9]+")
batch <- c()
for (l in lines) {
  print(l)
  batch <- c(batch, individual_info_df$batch[individual_info_df$cell.line == l])
}
...
save(GABA_mat, nmglut_mat, npglut_mat, file = "three_celltype_pseudobulk_count_mat_raw.RData")
save(GABA_mat_adj, nmglut_mat_adj, npglut_mat_adj, 
     file = "three_celltype_pseudobulk_count_mat_combatseq.RData")
...
for (i in 1:length(covar_df$cell_line)) {
  ...
  print(line)
  covar_df$age[i] <- individual_info_df$age[individual_info_df$cell.line == line]
  covar_df$sex[i] <- individual_info_df$sex[individual_info_df$cell.line == line]
  covar_df$batch[i] <- individual_info_df$batch[individual_info_df$cell.line == line]
}
...
save(covar_df, file = "029_covariates_full_df.RData")
...
design_GABA <- model.matrix(~0 + time + batch + age + sex + GABA_fraction, 
                            data = covar_df)
...
design_nmglut <- model.matrix(~0 + time + batch + age + sex + nmglut_fraction, 
                              data = covar_df)
...
design_npglut <- model.matrix(~0 + time + batch + age + sex + npglut_fraction, 
                              data = covar_df) 
