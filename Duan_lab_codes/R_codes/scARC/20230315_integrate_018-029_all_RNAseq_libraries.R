# Chuxuan Li 03/15/2023
# Get all libraries from 018 to 029, normalize them separately, then integrate
...
# 024
bc_path <- list.files(path = "/data/FASTQ/Duan_Project_024/hybrid_output",
                       full.names = T, pattern = ".best.tsv", recursive = T)
bc_path <- bc_path[str_detect(bc_path, "demux_0.01", T)]
barcode_list_add <- vector(mode = "list", length = length(bc_path))
for (i in 1:length(bc_path)){
  if (i == 1) {
...
load("../case_control_DE_analysis/redone_lines_char_vector.RData")

cleaned_lst <- human_only_lst
lib_indxs <- which(names(human_only_lst) %in% str_replace(groups_022, "_", "-"))
for (i in 1:length(lib_indxs)) {
  obj <- cleaned_lst[[lib_indxs[i]]]
  obj$cell.line.ident[obj$cell.line.ident %in% redone_lines] <- "redone"
  lines_in_obj <- unique(obj$cell.line.ident)
  print(lines_in_obj)
  if (length(lines_in_obj) == 1 & lines_in_obj == "redone") {
    cleaned_lst[[lib_indxs[i]]] <- NULL
  } else {
    cleaned_lst[[lib_indxs[i]]] <- subset(obj, cell.line.ident != "redone") 
  }
}  
...
save(integrated, file = "integrated_obj.RData")
