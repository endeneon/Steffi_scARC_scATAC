# Chuxuan Li 09/30/2021
# modify the Seurat DotPlot() function to accommodate more colors
...
DPmod <- function (object, assay = NULL, features, cols = c("lightgrey", 
          ...
          scale = TRUE, scale.by = "radius", scale.min = NA, scale.max = NA) 
{
  ...
  if (!is.null(x = split.by)) {
    ...
    if (split.colors) {
      print(length(cols))
      print((length(x = unique(x = splits))))
  ...
  if (split.colors) {
    ...
    # check what is splits.use
    #print(cols[splits.use])
    #cat("cols", cols)
    # changed from return(colorRampPalette(colors = c("grey", color))(20)[value])
    
    data.plot$colors <- mapply(FUN = function(color, value) {
      print(colorRampPalette(colors = c("#f2d0d0", "darkred"))(100)[value])
      # print(value)
      return(colorRampPalette(colors = c("#f2d0d0", "darkred"))(100)[value])
  ...
  #                mapping = aes_string(x = "features.plot", 
  #                                     y = "id")) + 
  #   geom_point(mapping = aes(size = pct.exp,
  #                            # # shape = 21L,
  #                            # color = "black",
  #                            fill = color.by),
  #              shape = 21,
  ...
  if (split.colors) {
    print("used scale color id")
    plot <- plot + scale_color_identity() #+
      #guides(color = guide_colorbar(title = "Average Expression"))

    # plot <- plot + 
    #   scale_point_color_gradient(low = "#F2D0D0", 
    #                                        high = "0000FF", 
    #                                        guide = guide_colourbar(available_aes = "point_colour"))
