# Chuxuan Li 10/3/2022
# count number of peaks shared by 2 or 3 time points for 18-line ATACseq to plot
#Venn diagram
...
txt_names <- sort(names(peaks_uncombined))
txt_counts <- matrix(nrow = 3, ncol = 4, dimnames = list(c("0hr", "1hr", "6hr"), 
                                           c("GABA", "NEFM_pos_glut", "NEFM_neg_glut", "Others")))
for (i in 1:length(txt_names)) {
  print(txt_names[i])
  txt_counts[i] <- sum(str_detect(gr$peak_called_in, txt_names[i]))
}
...
# count intersections ####
load("./macs2_called_new_peaks.RData")
...
times <- c("0hr", "1hr", "6hr")
for (i in 1:3) {
  timespec_names <- txt_names[str_detect(txt_names, times[i])]
