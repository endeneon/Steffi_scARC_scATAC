# Chuxuan Li 05/04/2023
# map to demuxlet generated barcodes and QC for 030 RNAseq data
...
h5list <- sort(list.files(path = "/data/FASTQ/Duan_Project_030",
                          ...
                          include.dirs = F, 
                          full.names = T))
h5list <- h5list[str_detect(h5list, "processed_hg38_mm10", negate = T)]
...
load("/nvmefs/scARC_Duan_018/Duan_project_030_RNA/Analysis_part1_mapped_to_hybrid_genome/removed_mouse_list_030_mapped_to_hybrid.RData")
...
names(barcode_list) <- str_remove(str_extract(pathlist, "[0-9]+_gex_[0|1|6]hr"), "gex_")
...
save(nomouselist, file = "GRCh38_mapped_removed_mouse_cells_assigned_demux_bc_list.RData")

# remove cells not matched to cell line barcodes
human_only_lst <- vector(mode = "list", length = length(nomouselist))
...
save(human_only_lst, file = "GRCh38_mapped_demux_matched_human_only_list.RData")
...
VlnPlot(rough_merged, features = c("nFeature_RNA", "nCount_RNA", "percent.mt"), 
        ncol = 3)
sum(rough_merged$percent.mt > 15) #6662
sum(rough_merged$nFeature_RNA > 7500) #488
sum(rough_merged$nCount_RNA > 40000) #352
sum(rough_merged$nFeature_RNA < 300) #272
sum(rough_merged$nCount_RNA < 500) #518
