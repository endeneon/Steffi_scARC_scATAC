...
# plot correlation heatmap of the cpm matrices of 030 RNA data.
...
for (k in 1:3){
  ...
  for (j in 1:length(types)) {
    ...
    pdf(file = paste0("./correlation_heatmaps/correlation_heatmap_by_line_and_type_", types[j], "_", t, ".pdf"))
    ...
    cor.mat <- cor.mat[order, order]
    melted_cor_mat <- melt(cor.mat)
    pdf(file = paste0("./correlation_heatmaps/correlation_heatmap_combat_corrected_", types[j], "_", t, ".pdf"))
    p <- ggplot(data = melted_cor_mat,
                aes(x = Var1, y = Var2, fill = value)) +
      geom_tile() +
      theme(axis.text.x = element_text(angle = 45, hjust = 0.55, vjust = 0.5, size = 8),
