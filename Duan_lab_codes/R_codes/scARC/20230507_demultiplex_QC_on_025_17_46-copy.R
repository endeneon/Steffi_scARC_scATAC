...
pathlist <- sort(list.files(path = ".", 
                            full.names = T, 
                            pattern = ".best.tsv", 
                            recursive = T))

barcode_list <- vector(mode = "list", 
                       length = 6)

for (i in 1:length(pathlist)){
  
  barcode_list[[i]] <- read.delim(pathlist[i], 
                                  header = F, 
                                  row.names = NULL)
...
names(barcode_list) <- 
  paste0("17_", str_remove(str_extract(pathlist, "[0-2].best"), 
                           "\\.best"))

# setwd("/data/FASTQ/Duan_Project_025_17_46/barcode_demux_output/common_barcodes_46")
# # /home/duhea/NVME/scARC_Duan_025_GRCh38/barcodes_demuxed_by_library
# pathlist <- sort(list.files(path = ".", 
#                             full.names = T, 
#                             pattern = ".best.tsv", 
#                             recursive = T))
...
# for (i in 4:6){
#   
#   barcode_list[[i]] <- read.delim(pathlist[j], 
#                                   header = F, 
#                                   row.names = NULL)
#   
#   colnames(barcode_list[[i]]) <- c("barcode", 
#                                    "line")
#   
#   j = j + 1
#   
# }
# 
# names(barcode_list)[4:6] <- 
#   paste0("46_", str_remove(str_extract(pathlist, "[0-2].best"), 
#                            "\\.best"))

# sep lines in each group's barcode list, then assign line by barcode
names(noratlist) <- str_extract(h5list, 
                                "[0-9]+-[0-6]")
...
for (i in 1:length(barcode_list)){
  ...
  for (j in 1:length(lines)){
    ...
    line_spec_barcodes <- 
      barcode_list[[i]]$barcode[barcode_list[[i]]$line == lines[j]]
    
    noratlist[[i]]$cell.line.ident[noratlist[[i]]@assays$RNA@counts@Dimnames[[2]] %in% 
                                     line_spec_barcodes] <- lines[j]
  }
  cell_counts$human[i] <- 
    sum(noratlist[[i]]$cell.line.ident != "unmatched")
  
  cell_counts$nonhuman[i] <- 
    sum(noratlist[[i]]$cell.line.ident == "unmatched")
}
...
write.table(cell_counts, 
            file = "cell_counts_based_solely_on_GRCh38_mapped_data_17_46.csv", 
            sep = ",", 
            quote = F, 
            col.names = T, 
            row.names = F)
...
for (i in 1:(length(human_only_lst) - 1)) {
  if (i == 1) {
    rough_merged <- merge(human_only_lst[[i]], 
                          human_only_lst[[i + 1]])
  } else {
    rough_merged <- merge(rough_merged, 
                          human_only_lst[[i + 1]])
  }
} 
