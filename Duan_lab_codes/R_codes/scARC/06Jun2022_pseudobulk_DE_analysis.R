...
# 5+18+20 line combined differential expression analysis using pseudobulk
...
# make df ####
load("~/NVME/scARC_Duan_018/Duan_project_024_RNA/Analysis_v4_combine_5_18_20line/5_18_20line_combined_labeled_nfeat5000_obj.RData")
...
# save results
save(res_nmglut_0v1, res_nmglut_0v6, res_nmglut_1v6,
     res_npglut_0v1, res_npglut_0v6, res_npglut_1v6,
     res_GABA_0v1, res_GABA_0v6, res_GABA_1v6, file = "deseq_res_dataframes.RData")
...
for (p in types){
  for (m in times){
    filenames <- c(filenames, paste("./", 
                                    p, "_", m, 
                                    "_DE_analysis_res_full_dataframe.csv", sep = ""))
  }
}
filenames
...
pathlist <- list.files("pseudobulk_DE/basemean_filtered_full_res", full.names = T,
                       pattern = "1v0|6v0")
pathlist
...
for (i in 1:length(reslist)) {
  ...
  pdf(paste0(namelist[i], 
             "_volcano_plot_v2.pdf"), 
      width = 8, 
      height = 4)
