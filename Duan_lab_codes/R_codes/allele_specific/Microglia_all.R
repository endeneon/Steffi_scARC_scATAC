...
df_raw <-
  read.vcfR(file = "../Hanwen_ATACseq_test/MGs/dup_keep/merge/output/MG_11May2023_untrimmed_merged_SNP_VF.vcf",
            limit = 1e9,
            verbose = T)
...
write.table(df_assembled,
            file = "microglia_38_lines_all_SNPs_18May2023.txt",
            quote = F, sep = "\t",
...
ggplot(df_assembled,
  ...
  ggtitle(paste("Microglia, DP >= 20, minAllele >= 2, 38 lines;\n",
...
write.table(df_assembled_FDR_005,
            file = "microglia_38_lines_005_18May2023.txt",
            quote = F, sep = "\t",
            row.names = F, col.names = T)

save.image("microglia_38_lines_11May2023.RData")

df_avinput <-
  df_assembled_FDR_005[, c(1,2,2,4,5,3,6:ncol(df_assembled_FDR_005))]
write.table(df_avinput,
            file = "microglia_38_lines_005_18May2023.avinput",
            quote = F, sep = "\t",
            row.names = F, col.names = T)
...
## make a txt for circos
df_4_circos <-
  df_assembled[, c(1,2,2,15)]
df_4_circos <-
  df_4_circos[df_4_circos$CHROM %in% c("chr2",
                                       "chr5",
                                       "chr7",
                                       "chr8",
                                       "chr10",
                                       ...
                                       "chr20"), ]
df_4_circos$POS <-
  as.numeric(df_4_circos$POS) - 1
df_4_circos <-
  df_4_circos[order(df_4_circos$POS, df_4_circos$CHROM), ]
# df_4_circos$POS <-
#   format(df_4_circos$POS,
#          scientific = F)

# make sure write.table will not send 10e format
options(scipen = 20)
write.table(df_4_circos,
            file = "MG_all_SNP_4_circos.txt",
            quote = F,
            row.names = F, col.names = F)
options(scipen = 0)

max(df_4_circos$`-logFDR`)
