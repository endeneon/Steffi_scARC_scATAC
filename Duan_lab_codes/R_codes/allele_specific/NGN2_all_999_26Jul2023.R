...
df_raw <-
  read.vcfR(file = "Aug2023_vcfs/NGN2_18Aug2023_trimmed_merged_SNP_VF.vcf",
            limit = 2e9,
            verbose = T)
...
### require DP >= 40 #####
# df_assembled <-
#   df_assembled[df_assembled$DP > 25, ] # 20 to 25
df_assembled <-
  df_assembled[!is.na(df_assembled$DP), ]

df_assembled <-
  df_assembled[df_assembled$DP > 40, ]


# rs2027349
df_assembled[df_assembled$ID == "rs2027349", ]
# REF_N+ 21, 18, 6
# # ALT_N+ 75, 80, 18
# df_assembled[df_assembled$ID == "rs2027349", 10] <-
#   283 + sum(21, 18, 6)
# df_assembled[df_assembled$ID == "rs2027349", 11] <-
#   450 + sum(75, 80, 18)
# df_assembled[df_assembled$ID == "rs2027349", 12] <-
#   283 + 450 + sum(21, 18, 6,75, 80, 18)
...
# save.image("NGN2_36_lines_26Jul2023.RData")
...
write.table(df_assembled,
            file = "NGN2_36_lines_all_SNPs_25Sept2023.txt",
            quote = F, sep = "\t",
...
write.table(df_assembled_FDR_005,
            file = "NGN2_36_lines_005_25Aug2023.txt",
            quote = F, sep = "\t",
            row.names = F, col.names = T)
...
write.table(df_avinput,
            file = "NGN2_36_lines_005_25Aug2023.avinput",
            quote = F, sep = "\t",
            row.names = F, col.names = T)

df_assembled <-
  readRDS("NGN2_36_lines_25Sept2023.RData")
...
ggplot(df_assembled,
  ...
  ggtitle(paste("NGN2-Glut, DP >= 40, minAllele >= 2, 36 lines;\n",
...
saveRDS(object = df_assembled,
        file = "NGN2_36_lines_25Sept2023.RData")

## extract the line with rs2027349
sum_gt_AD <- as.data.frame(sum_gt_AD)
sum_gt_AD[1, ]
rownames(sum_gt_AD)
df_rs2027349 <-
  sum_gt_AD[rownames(sum_gt_AD) %in% "rs2027349", ]
colnames(df_rs2027349) <-
  str_split(string = colnames(df_rs2027349),
            pattern = "_S",
            simplify = T)[, 1]
df_rs2027349

df_test <-
  sum_gt_AD[rownames(sum_gt_AD) %in% "rs1532278", ]
colnames(df_test) <-
  str_split(string = colnames(df_test),
            pattern = "_S",
            simplify = T)[, 1]
df_test
