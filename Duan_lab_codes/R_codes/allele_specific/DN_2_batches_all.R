...
df_raw <-
  read.vcfR(file = "Aug2023_vcfs/DN_21Aug2023_trimmed_merged_SNP_VF.vcf",
            limit = 2e9,
            verbose = T)
...
write.table(df_assembled,
            file = "DN_39_lines_all_SNPs_25Sept2023.txt",
            quote = F, sep = "\t",
...
df_assembled <-
  readRDS("DN_39_lines_25Sept2023.RDs")
...
ggplot(df_assembled,
  ...
  ggtitle(paste("Dopa, DP >= 40, minAllele >= 2, 39 lines;\n",
...
ggplot(df_assembled,
  ...
  scale_color_manual(values = c("red", "black")) +
  labs(colour = "FDR value") +
  ggtitle(paste("Dopa, DP >= 20, minAllele >= 2, 39 lines;\n",
                # "SNP lists from 23 Sept results, removed all NCALLED >= 14\n",
                nrow(df_assembled),
                "SNPs, of",
                sum(df_assembled$FDR < 0.05),
...
write.table(df_assembled_FDR_005,
            file = "DN_39_lines_005_21Aug2023.txt",
            quote = F, sep = "\t",
            row.names = F, col.names = T)
...
saveRDS(object = df_assembled,
        file = "DN_39_lines_25Sept2023.RDs")
