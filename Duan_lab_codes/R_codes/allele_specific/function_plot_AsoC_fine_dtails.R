...
plot_AsoC_fine_dtails <- function(chr, start, end, gene_name = "",
                            ...
                            cell_type_to_plot = "CN", title_name = "", pileup_colour = "darkolivegreen4",
  ...
  alTrack_Region <- AlignmentsTrack(paste("rs2027349_het_BAMs/", cell_type_to_plot, "_het_rs2027349.bam",
                                          collapse = "", sep = ""),
                                    isPaired = T, coverageOnly = T,
                                    chromosome = as.character(unique(seqnames(cell_type))),
                                    genome = "hg38", type = "coverage",
                                    background.title = pileup_colour,
                                    fill.coverage = pileup_colour,
                                    col.coverage = pileup_colour,
                                    ylim = c(0, ylimit),
                                    transformation=function(x) {x * transformation_factor},
                                    cex.axis = 1,
                                    col.title="black",
                                    col.axis="black",
                                    name = title_name,
  ...
  alTrack_A <- AlignmentsTrack(paste("rs2027349_het_BAMs/SNP_split_output/", cell_type_to_plot, "_het_rs2027349.ref.A.bam",
                                     collapse = "", sep = ""),
                               isPaired = T, coverageOnly = T,
                               chromosome = as.character(unique(seqnames(cell_type))),
                               genome = "hg38", type = "coverage",
  ...
  alTrack_G <- AlignmentsTrack(paste("rs2027349_het_BAMs/SNP_split_output/", cell_type_to_plot, "_het_rs2027349.alt.G.bam",
                                     collapse = "", sep = ""),
                               isPaired = T, coverageOnly = T,
                               chromosome = as.character(unique(seqnames(cell_type))),
                               genome = "hg38", type = "coverage",
  ...
  grTrack@dp@pars$fontsize.group <- 14
